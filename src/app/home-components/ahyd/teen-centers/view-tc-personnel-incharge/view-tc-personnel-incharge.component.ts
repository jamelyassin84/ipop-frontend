...
export class ViewTcPersonnelInchargeComponent implements OnInit {
	constructor(private service: PersonnelInchargeOfTeenCenterService, private component: ReloadService) {
	...
	ngOnInit(): void {
		setTimeout(() => {
			new BaseService(this.service.http, this.service.url, `sbmptc_id=${this.teen_center_id}`).index().subscribe((data: any) => {
